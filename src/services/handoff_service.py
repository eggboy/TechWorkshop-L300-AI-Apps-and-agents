...
class IntentClassification(BaseModel):
    ...
    is_domain_change: bool = Field(description="Whether this represents a change from the current domain")
...
AGENT_DOMAINS = {
    "cora": {"name": "Cora Shopping Assistant", "description": "General shopping assistance, product browsing"},
    "interior_designer": {
    ...
    def classify_intent(self, user_message: str, session_id: str, chat_history: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if intent["is_domain_change"]:
                ...
                logger.info(
                    f"[HANDOFF_SERVICE] Domain change for session {session_id}: {current_domain} -> {intent['domain']}"
                )
